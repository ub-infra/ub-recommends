...
const SkinAnalysis = ({ onClick }: SkinAnalysisProps) => {
  ...
  const [skinResults, setSkinResults] = useState<any>(resultExample);
  ...
  const fetchSkinAnalysis = (data: any) => {
    // const url = `https://app.unsweetenedbeauty.com/skin/analysis`;
  ...
  return (
    <div>
      ...
      {step < 1 && <p style={{ fontSize: 12, color: "grey", fontWeight: 300 }}>
        {index == 3
          ? "Your face analysis is ready. Check out your product matches now."
          : "Get product matches via detailed face analysis."}
      </p>}
      {step > 0 ? (
        <SelectFavouriteProducts products={productsToSelect} onClick={onClick} />
      ) : (
        <>
          {index < 3 ? (
            <WebcamCapture
